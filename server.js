  ...
  if (playersCount.rows[0].count === 0) {
    ...
    for (const player of players) {
      ...
      await pool.query(
        ...
        (name, number, position, team_id, goals, assists, saves)
...
app.get("/api/statistics", async (req, res) => {
  ...
  const result = await pool.query(`
    ...
    ORDER BY players.goals DESC, players.assists DESC, players.id ASC
...
app.put("/api/players/:id/stats", requireAdmin, async (req, res) => {
  ...
  const safeGoals = Math.max(0, Number(goals) || 0);

  const safeAssists = Math.max(0, Number(assists) || 0);
...
app.get("/api/team-of-week", async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({
      ...
      error: "Could not load team of the week"
...
app.post("/api/team-of-week", requireAdmin, async (req, res) => {
  ...
  try {
    ...
    const selectedPlayers = await pool.query(
    ...
    if (selectedPlayers.rows.length !== 11) {
    ...
    const preparedPlayers = players.map(player => {
      ...
      const selectedPosition =
        ...
        player.position
          ...
          : getPositionGroup(dbPlayer.position);
    ...
    if (goalkeeperCount !== 1) {
      ...
      return res.status(400).json({
        ...
        error: "Team of the Week must contain exactly 1 goalkeeper"
    ...
    if (
      ...
      return res.status(400).json({
        ...
        error: "Invalid player positions"
    ...
    res.status(500).json({
      ...
      error: "Could not save team of the week"
