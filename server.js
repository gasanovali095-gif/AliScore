...
async function initDatabase(){
  ...
  const n=(await pool.query("SELECT COUNT(*)::int count FROM teams")).rows[0].count;
  if(!n){
    for(const name of ["Xirdalan United","Xirdalan Wolves","Neweli FK","MSN FK","Lotu pişiklər"])
...
app.post("/api/events",admin,async(req,res)=>{
  try{
    ...
    if(type==="goal"){
      ...
      await pool.query(`UPDATE matches SET home_score=home_score+CASE WHEN home_team_id=$1 THEN 1 ELSE 0 END, away_score=away_score+CASE WHEN away_team_id=$1 THEN 1 ELSE 0 END WHERE id=$2`,[team_id,match_id]);
    }
