...
async function initDb() {
  ...
  const count = await pool.query("SELECT COUNT(*)::int AS count FROM teams");
...
function auth(req, res, next) {
  ...
  if (!token) return res.status(401).json({ error: "Admin giriş tələb olunur" });
  try {
    req.admin = jwt.verify(token, JWT_SECRET);
    next();
  } catch {
    return res.status(401).json({ error: "Admin sessiyası bitib" });
  }
}
...
app.get("/api/teams", async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT id, name, played, wins, draws, losses, goals_for, goals_against,
...
app.post("/api/teams", auth, async (req, res) => {
  ...
  try {
    ...
    res.status(400).json({ error: "Bu komanda artıq mövcuddur" });
  }
});
...
app.put("/api/teams/:id", auth, async (req, res) => {
  ...
  try {
    ...
    if (!result.rowCount) return res.status(404).json({ error: "Komanda tapılmadı" });
    res.json(result.rows[0]);
  } catch {
    res.status(400).json({ error: "Bu komanda adı artıq istifadə olunur" });
  }
});
...
app.get("/api/matches", async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      ORDER BY
        CASE WHEN m.home_score IS NULL OR m.away_score IS NULL THEN 0 ELSE 1 END,
...
app.post("/api/matches", auth, async (req, res) => {
  ...
  if (!homeTeamId || !awayTeamId || homeTeamId === awayTeamId) {
    return res.status(400).json({ error: "İki fərqli komanda seçin" });
  }
  try {
    const result = await pool.query(
    ...
    res.status(400).json({ error: "Matç yaradıla bilmədi" });
  }
});

app.put("/api/matches/:id", auth, async (req, res) => {
  const id = Number(req.params.id);
  const homeScore = req.body?.homeScore === "" || req.body?.homeScore == null
    ? null : Number(req.body.homeScore);
  const awayScore = req.body?.awayScore === "" || req.body?.awayScore == null
    ? null : Number(req.body.awayScore);
  const matchDate = String(req.body?.matchDate || "").trim();

  if ((homeScore !== null && (!Number.isInteger(homeScore) || homeScore < 0)) ||
      (awayScore !== null && (!Number.isInteger(awayScore) || awayScore < 0))) {
    return res.status(400).json({ error: "Hesab 0 və ya daha böyük tam ədəd olmalıdır" });
  }

  try {
    const result = await pool.query(
      `UPDATE matches
       SET home_score=$1, away_score=$2, match_date=$3
       WHERE id=$4 RETURNING *`,
      [homeScore, awayScore, matchDate, id]
    );
    if (!result.rowCount) return res.status(404).json({ error: "Matç tapılmadı" });
...
async function recalculateStandings() {
  ...
  try {
    ...
    for (const m of matches.rows) {
      ...
      await client.query(
        `UPDATE teams
         SET played=played+1, goals_for=goals_for+$1, goals_against=goals_against+$2
         WHERE id=$3`,
        [hs, as, h]
      );
      await client.query(
      ...
      if (hs > as) {
        await client.query("UPDATE teams SET wins=wins+1 WHERE id=$1", [h]);
        await client.query("UPDATE teams SET losses=losses+1 WHERE id=$1", [a]);
      } else if (hs < as) {
        await client.query("UPDATE teams SET losses=losses+1 WHERE id=$1", [h]);
        await client.query("UPDATE teams SET wins=wins+1 WHERE id=$1", [a]);
      } else {
        await client.query("UPDATE teams SET draws=draws+1 WHERE id=$1", [h]);
        await client.query("UPDATE teams SET draws=draws+1 WHERE id=$1", [a]);
      }
    }
